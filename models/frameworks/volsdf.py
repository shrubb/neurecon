...
def fine_sample(implicit_surface_fn, init_dvals, rays_o, rays_d,
    ...
    with torch.no_grad():
        ...
        final_fine_dvals = torch.zeros([*prefix, final_N_importance], device=device)
        final_iter_usage = torch.zeros([*prefix], device=device)
        ...
        while it_algo < max_iter:
            ...
            if mask.sum() > 0:
                ...
                d_vals = torch.cat([d_vals, torch.zeros([*prefix, N_up], device=device)], dim=-1)
                sdf = torch.cat([sdf, torch.zeros([*prefix, N_up], device=device)], dim=-1)
...
def volume_render(
    ...
    **dummy_kwargs  # just place holder
    ...
    def render_rayschunk(rays_o: torch.Tensor, rays_d: torch.Tensor):
        ...
        # ---------------
        # Coarse Points
        _t = torch.linspace(0, 1, N_samples, device=device)
        # [(B), N_rays, N_samples]
        ...
        with torch.no_grad():
            ...
            _t = torch.linspace(0, 1, N_samples, device=device) # NOTE: you might want to use less samples for faster training.
            ...
            d_fine, beta_map, iter_usage = fine_sample(
                ...
                N_up=N_samples    # NOTE: you might want to use less samples for faster training.
            )
        ...
        radiances, sdf, nablas = batchify_query(model, pts, view_dirs.unsqueeze(-2).expand_as(pts) if use_view_dirs else None)
        # [(B), N_rays, N_pts]
        sigma = sdf_to_sigma(sdf, alpha, beta)
        ...
        # NeRF++
        if use_nerfplusplus:
            _t = torch.linspace(0, 1, N_outside + 2, device=device)[..., 1:-1]
            ...
            if perturb:
                ...
                _t_rand = torch.rand(_upper.shape, device=device)
            ...
            sigma_out, radiance_out = batchify_query(model.nerf_outside, x_out, view_dirs.unsqueeze(-2).expand_as(pts_out) if use_view_dirs else None)
...
class Trainer(nn.Module):
    ...
    def forward(self,
        ...
        eikonal_points = torch.empty_like(nablas, device=device).uniform_(-eik_bounding_box, eik_bounding_box)
